...
def _header(win, subtitle=None):
    ...
    if subtitle:
        tk.Label(
            win, text=subtitle, font=("Courier New", 8), fg=TEXTDIM, bg=BG2, padx=20
...
class SetupDialog(tk.Toplevel):
    def __init__(self, parent, existing=None):
        ...
        for idx in range(num_inst):
            ...
            var = tk.StringVar(
                value=cfg.get(key, f"instance {idx + 1}" if idx >= 3 else "")
            )
            self.fields[key] = var
            tk.Entry(
                row,
    ...
    def _save(self):
        ...
        if not cfg["adb_path"] or not cfg["mumu_exe"]:
            messagebox.showerror(
                "Missing fields", "Both adb.exe and MuMuManager.exe paths are required."
            )
            return
...
class InstancePicker(tk.Toplevel):
    def __init__(self, parent):
        ...
        row.pack(fill="x")
        tk.Label(
            row, text="Count:", font=("Courier New", 9, "bold"), fg=TEXT, bg=BG
    ...
    def _confirm(self):
        ...
        for i in range(len(self._known), n):
            name = (
                self._extra.get(i, tk.StringVar(value=f"instance {i + 1}"))
                .get()
                .strip()
            )
            if not name:
...
class RobloxGuardianApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(
            bar, text="v2.0", font=("Courier New", 9), fg=TEXTDIM, bg=BG2, padx=4
        ).pack(side="left", pady=14)
        tk.Label(
            bar,
            text=f"[ {len(self.instances)} instances ]",
            font=("Courier New", 8, "bold"),
            fg=YELLOW,
            bg=BG2,
            padx=4,
        ...
        scroll_frame.bind(
            "<Configure>",
            lambda e: (_on_frame_configure(e), _bind_scroll(scroll_frame)),
        )
        ...
        for inst in self.instances:
            self._cards.append(
                self._make_card(scroll_frame, inst["name"], inst["port"])
            )
    ...
    def _make_card(self, parent, name, port):
        ...
        dot.pack(side="left")
        tk.Label(
            top, text=f"  {name}", font=("Courier New", 9, "bold"), fg=TEXT, bg=BG3
        ).pack(side="left")
        st = tk.Label(
            top, text="IDLE", font=("Courier New", 8, "bold"), fg=TEXTDIM, bg=BG3
        )
        st.pack(side="right")
        tk.Label(
            f, text=f"port {port}", font=("Courier New", 7), fg=TEXTDIM, bg=BG3
        ).pack(anchor="w", pady=(2, 0))
        pid = tk.Label(f, text="PID: —", font=("Courier New", 7), fg=TEXTDIM, bg=BG3)
        pid.pack(anchor="w")
        return {"dot": dot, "status": st, "pid": pid}
    ...
    def _make_stat(self, parent, label, var):
        ...
        tk.Label(
            row, textvariable=var, font=("Courier New", 8, "bold"), fg=TEXT, bg=BG
        ).pack(side="left")
    ...
    def _log(self, msg, color=TEXT):
        def _write():
            ...
            tag = {GREEN: "ok", RED: "fail", YELLOW: "warn", ACCENT: "info"}.get(
                color, "normal"
            )
    ...
    def _adb_s(self, port, *args, capture=False):
        """ADB with -s serial."""
        return self._run(
            [self._adb_exe, "-s", f"127.0.0.1:{port}", *args], capture=capture
        )
        ...
        try:
            r = subprocess.run(
                [
                    "tasklist",
                    "/FI",
                    "IMAGENAME eq MuMuVMMHeadless.exe",
                    "/FO",
                    "CSV",
                    "/NH",
                ],
    ...
    def _is_alive(self, inst):
        ...
        if not line or "offline" in line:
            # One retry with connect
            self._adb("connect", serial, capture=True)
            self._sleep(1)
            devices = self._adb("devices", capture=True) or ""
            line = next((l for l in devices.splitlines() if serial in l), "")
            if not line or "offline" in line:
    ...
    def _launch_roblox(self, inst):
        ...
        port = inst["port"]
        idx = inst["index"]
        self._adb_s(port, "shell", "am", "force-stop", "com.netease.mumu.gamecenter")
        self._sleep(1)
        self._mumu(
            "api",
            "-v",
    ...
    def _recover(self, inst, card_idx):
        ...
        self._mumu(
            "api", "-v", str(idx), "shell", "am", "force-stop", "com.roblox.client"
        )
        self._sleep(1)

        self._log(f"  [2/5] Closing {name}...", YELLOW)
        self._mumu("api", "-v", str(idx), "shutdown")
        ...
        self._mumu("api", "-v", str(idx), "launch")
        ...
        if alive:
            ...
            self._log(
                f"  [WARN] {name} still not responding, retry next check.", YELLOW
            )
            self._set_status("● MONITORING (degraded)", YELLOW)
    ...
    def _set_startup(self, enable):
        try:
            ...
            if enable:
                ...
                try:
                    winreg.DeleteValue(key, "RobloxGuardian")
    ...
    def _do_afk_restart(self):
        if not AFK_FILE.exists():
            return
        self._flog("AFK: 2h restart triggered")
        subprocess.run(
            ["shutdown", "/r", "/t", "10", "/c", "Roblox Guardian AFK restart"]
        )
    ...
    def _worker(self):
        ...
        if all_up:
            ...
            for inst in self.instances:
                ...
                self._mumu("api", "-v", str(inst["index"]), "launch")
                self._sleep(1)
        ...
        while not self._stop.is_set():
            ...
            if not recovered:
                self._log(
                    f"  All {len(self.instances)} OK. Next check in {CHECK_INTERVAL}s.",
                    GREEN,
                )
