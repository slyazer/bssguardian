...
def validate_config_paths(cfg):
    ...
    ok = Path(cfg.get("adb_path", "")).exists() and Path(cfg.get("mumu_exe", "")).exists()
...
class RobloxGuardianApp:
    def __init__(self, root, instances, cfg):
        ...
        self._disabled  = set()   # ports manually shut down by user
    ...
    def _build_ui(self):
        ...
        # Fixed buttons at bottom (pack before canvas so they get space first)
    ...
    def _is_alive(self, inst):
        """
        3-step check:
        1. MuMu process count
        2. ADB device online (3 retries with fresh disconnect/connect)
        3. pidof com.roblox.client + ANR check
        ...
        # 2. ADB online? — 3 retries with full disconnect/reconnect each time
        adb_ok = False
        for attempt in range(3):
            ...
            if line and "offline" not in line and "unauthorized" not in line:
                ...
                break
            if line:
                self._log(f"    [DBG:{port}] ADB attempt {attempt+1}: {line}", TEXTDIM)
            self._sleep(1)

        if not adb_ok:
            self._log(f"    [DBG:{port}] DEAD — ADB offline after 3 retries", TEXTDIM)
            return False, ""
    ...
    def _launch_roblox(self, inst):
        port = inst["port"]
        idx  = inst["index"]
        self._mumu("control", "app", "close", "--package", "com.roblox.client", "-v", str(idx))
        self._sleep(2)
    ...
    def _recover(self, inst, card_idx):
        """Full restart sequence for a crashed instance."""
    ...
    def _manual_launch(self, inst):
        """Manually boot an instance + launch Roblox, re-enable monitoring."""
        name, idx, port = inst["name"], inst["index"], inst["port"]
        ...
        self._adb("start-server")
        if not self._wait_adb(port, timeout=60):
            self._log(f"  [WARN] ADB offline for {name}, aborting.", RED)
        ...
        self._set_status(
            "● MONITORING" if self.running else "● IDLE",
            GREEN if self.running else TEXTDIM
        )

    def _manual_shutdown(self, inst):
        """Manually shut down an instance and pause monitoring for it."""
        name, idx, port = inst["name"], inst["index"], inst["port"]
        ...
        self._log(f"  Shutdown command sent for {name}", GREEN)
        for _ in range(20):
            if self._stop.is_set(): return
            if self._mumu_count() <= idx:
                break
        ...
        if card_idx is not None:
            def _mark_disabled(ci=card_idx):
                card = self._cards[ci]
    ...
    def _worker(self):
        ...
        # Initial ADB connect for all instances
        self._adb("start-server")
        self._sleep(3)
        for inst in self.instances:
            serial = f"127.0.0.1:{inst['port']}"
        ...
        # Fast-path: are all instances already up?
        all_up = True
        ...
        while not self._stop.is_set():
            ...
            # FIX: fresh disconnect+reconnect per instance each cycle
            for inst in self.instances:
                serial = f"127.0.0.1:{inst['port']}"
                self._adb("disconnect", serial)
                self._sleep(0.3)
            ...
            for i, inst in enumerate(self.instances):
                ...
                # Skip instances manually shut down by user
                ...
                if not alive:
                    # FIX: call _recover (not _manual_shutdown) and skip OK log
                    ...
                    recovered = True
                    continue   # FIX: don't fall through to the OK log below
